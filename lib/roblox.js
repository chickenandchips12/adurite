...
async function validateRobloxCookie(cookie) {
  ...
  if (cookieValue.includes("_|WARNING:")) {
    cookieValue = cookieValue.split("_|WARNING:")[0].trim();
  }
  ...
  if (!res.ok) {
    if (res.status === 401) {
      throw new Error(
        "Invalid or expired cookie. Try: 1) Log out of Roblox and log back in, 2) Get a fresh cookie from roblox.com, 3) Copy the full value (double-click to select all)"
      );
    }
